...
for lines in f:
    ...
    for i in ls:
        ...
        while '' in list_n:
            list_n.remove('')
        li1=np.array(list_n)[-16:-14]
        li2 = np.array(list_n)[0:12]
        s00.append(li1)
        s01.append(li2)
        # s.append(i.split(" "))
f.close()
...
data_n=np.c_[data_n,s00]     #去掉电场矢量参数
...
datam=DataFrame(s11)
data_m=np.array(datam.loc[:,[1,2,8,9,10,11]])
data_m=np.c_[data_m,s10]          #去掉电场矢量参数
...
scale=np.std(data_n,0)*5 #求无人矩阵各个参数的标准差*(10%、20%...)
